...
router.patch("/:id",requireRoles("merchant_admin","merchant_staff","super_admin"),asyncHandler(async(req,res)=>{
  const id=z.coerce.number().int().positive().parse(req.params.id);const input=bodySchema.omit({doctorId:true}).partial().refine(v=>Object.keys(v).length>0,"至少提供一个修改字段").parse(req.body);const [rows]=await pool.query<RowDataPacket[]>("SELECT hospital_id hospitalId,booked_slots bookedSlots FROM doctor_schedules WHERE id=? AND status=1",[id]);if(!rows[0])throw new AppError(404,"SCHEDULE_NOT_FOUND","排班不存在");await assertHospitalAccess(req.auth!,rows[0].hospitalId);if(input.totalSlots!==undefined&&input.totalSlots<rows[0].bookedSlots)throw new AppError(409,"INVALID_TOTAL_SLOTS","总号源不能小于已预约数");
