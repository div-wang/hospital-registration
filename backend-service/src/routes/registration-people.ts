...
const bodySchema = z.object({
  name: z.string().trim().min(2).max(64), phone: phoneSchema, idCard: idCardSchema,
...
const mapPerson = (row: RowDataPacket) => ({
  id: row.id, name: row.name, phone: row.phone, idCardMasked: maskIdCard(row.idCardLast4),
...
router.get("/", asyncHandler(async (req, res) => {
  ...
  let keywordClause = "";
  if (query.keyword) { keywordClause = " AND (name LIKE ? OR phone LIKE ?)"; params.push(`%${query.keyword}%`, `%${query.keyword}%`); }
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT id, name, phone, id_card_last4 idCardLast4, relationship, is_default isDefault,
...
router.post("/", asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.execute<ResultSetHeader>(
      `INSERT INTO registration_people (user_id, name, phone, id_card_ciphertext, id_card_hash, id_card_last4, relationship, is_default)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [req.auth!.userId, input.name, input.phone, encryptSensitive(input.idCard), hash, input.idCard.slice(-4), input.relationship, input.isDefault ? 1 : 0],
    );
...
router.patch("/:id", asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await connection.execute(
      `UPDATE registration_people SET name=?, phone=?, id_card_ciphertext=?, id_card_hash=?, id_card_last4=?, relationship=?, is_default=? WHERE id=?`,
      [input.name ?? current.name, input.phone ?? current.phone, idCard ? encryptSensitive(idCard) : current.id_card_ciphertext,
