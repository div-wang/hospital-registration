...
type Dept = { name: string; icon: string; tone: string };

const departments: Dept[] = [
  { name: "内科", icon: "🫁", tone: "mint" },
  { name: "外科", icon: "✚", tone: "blue" },
  { name: "妇产科", icon: "❀", tone: "pink" },
  { name: "儿科", icon: "☻", tone: "yellow" },
  { name: "皮肤科", icon: "◌", tone: "purple" },
  { name: "中医科", icon: "♨", tone: "orange" },
  { name: "口腔科", icon: "♢", tone: "cyan" },
  { name: "更多科室", icon: "•••", tone: "gray" },
];
    ...
    <main className="app-shell">
      <div className="phone">
        <header className="topbar">
          <button className="city" aria-label="选择城市">北京 <span>⌄</span></button>
          <div className="brand"><span className="brand-mark">✚</span><b>安康挂号</b></div>
          <button className="message" aria-label="消息" onClick={() => notify("暂时没有新消息")}>◔<i /></button>
        </header>
        ...
        {tab === "home" && <>
          <section className="hero">
            ...
            <button className="search" onClick={() => setSearchOpen(true)}><span>⌕</span><span>搜索医院、科室、医生或疾病</span></button>
            <div className="hero-art" aria-hidden="true"><div className="cross">✚</div><div className="pulse">⌁</div></div>
          </section>

          <section className="quick-grid">
            <button onClick={() => { setSelectedDept("全部"); setTab("guide"); }}><span className="quick-icon blue">▥</span><b>按医院挂号</b><small>查看附近医院</small></button>
            <button onClick={() => setTab("guide")}><span className="quick-icon green">♟</span><b>按医生挂号</b><small>匹配专业医生</small></button>
            <button onClick={() => { setSelectedDept("内科"); setTab("guide"); }}><span className="quick-icon violet">✦</span><b>智能导诊</b><small>症状自查推荐</small></button>
            <button onClick={() => setTab("orders")}><span className="quick-icon orange">▣</span><b>报告查询</b><small>检查结果随时看</small></button>
          </section>

          <section className="section department-section">
            <div className="section-head"><div><p>快速预约</p><h2>热门科室</h2></div><button onClick={() => setTab("guide")}>全部科室 <span>›</span></button></div>
            <div className="dept-grid">
              {departments.map(d => <button key={d.name} onClick={() => { if(d.name !== "更多科室") setSelectedDept(d.name); setTab("guide"); }}><span className={`dept-icon ${d.tone}`}>{d.icon}</span><b>{d.name}</b></button>)}
            </div>
          </section>

          <section className="section hospital-section">
            <div className="section-head"><div><p>优质资源</p><h2>附近好医院</h2></div><button onClick={() => setTab("guide")}>查看全部 <span>›</span></button></div>
            <div className="hospital-scroll">
              {hospitals.map((h, i) => <button className="hospital-card" key={h.name} onClick={() => { setTab("guide"); notify(`已选择${h.name}`); }}><span className="hospital-logo" style={{background:h.color}}>{i === 0 ? "协" : i === 1 ? "北" : "同"}</span><div><h3>{h.name}</h3><p><i>{h.level}</i>{h.focus}</p><small>可约号源 · {h.distance}</small></div><span className="arrow">›</span></button>)}
            </div>
          </section>

          <section className="health-card">
            <div><span>今日健康贴士</span><h3>春夏交替，注意过敏防护</h3><p>花粉季外出建议佩戴口罩，回家后及时清洁面部。</p></div><div className="leaf">⌁</div>
          </section>
        </>}

        {tab === "guide" && <section className="subpage">
          <div className="page-title"><button onClick={() => setTab("home")}>‹</button><h1>预约挂号</h1><span /></div>
          <button className="search light" onClick={() => setSearchOpen(true)}><span>⌕</span><span>搜索疾病、科室、医生、医院</span></button>
          <div className="segmented"><button className="active">找医生</button><button onClick={() => notify("已切换为医院视图")}>找医院</button></div>
          <div className="filter-row"><button>北京市⌄</button><button>{selectedDept === "全部" ? "全部科室" : selectedDept}⌄</button><button>综合排序⌄</button></div>
        ...
        {tab === "profile" && <section className="subpage profile-page">
          ...
          <div className="family"><div className="section-head"><div><p>就诊管理</p><h2>我的就诊人</h2></div><button onClick={() => notify("可以添加新的就诊人")}>添加 <span>＋</span></button></div><div className="family-person"><span>安</span><div><b>安先生</b><small>本人 · 已绑定医保</small></div><i>默认</i></div></div>
          <div className="menu-list">{[["▣","我的报告"],["☆","收藏医生"],["♧","健康档案"],["⌘","地址管理"],["?","帮助与客服"]].map(x => <button key={x[1]} onClick={() => notify(`${x[1]}功能已打开`)}><span>{x[0]}</span><b>{x[1]}</b><i>›</i></button>)}</div>
        </section>}

        <nav className="bottom-nav">
          {[{id:"home",icon:"⌂",label:"首页"},{id:"guide",icon:"✚",label:"挂号"},{id:"orders",icon:"▣",label:"预约"},{id:"profile",icon:"♙",label:"我的"}].map(item => <button key={item.id} className={tab === item.id ? "active" : ""} onClick={() => setTab(item.id as Tab)}><span>{item.icon}</span><small>{item.label}</small></button>)}
        </nav>

        {searchOpen && <div className="modal-backdrop" onClick={() => setSearchOpen(false)}><div className="search-modal" onClick={e => e.stopPropagation()}><div className="modal-handle"/><div className="modal-search"><span>⌕</span><input autoFocus value={query} onChange={e => setQuery(e.target.value)} placeholder="搜索医院、科室、医生或疾病"/><button onClick={() => setSearchOpen(false)}>取消</button></div><h3>热门搜索</h3><div className="hot-search">{["北京协和医院","皮肤科","儿童发热","睡眠障碍","口腔科"].map(x => <button key={x} onClick={() => {setQuery(x); notify(`正在搜索“${x}”`); setSearchOpen(false);}}>{x}</button>)}</div></div></div>}
